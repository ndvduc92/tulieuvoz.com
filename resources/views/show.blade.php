...
<div class="container">
    ...
    <div class="panel panel-default">
      ...
      <div class="panel-body">
        ...
        <ul>
          ...
          @if ($link->user_id)
          <li>Người đăng: <a class="label label-success" href="/user/{{ $link->user_id }}">{{ $link->user->name }}</a>
          </li>
          @else
          <li>Người đăng: Khách vãng lai</li>
          @endif
          <li>Mô tả: <span style="color:green">{{ $link->description ? $link->description : 'Chưa cập nhật' }}</span>
          </li>
          <li>Số lượt xem: {{ $link->viewed }}</li>
          <li>Like/Dislike: <span style="color:green"><i class="fa fa-thumbs-up"></i> {{ $link->like }}</span> <span
              style="color:red"><i class="fa fa-thumbs-down"></i> {{ $link->dislike }}</span></li>
  ...
  " class="fa fa-eye" onclick="showLink()"></i></b></li>
        ...
        <div id="showLink" style="display:none">
          ...
          <p>
            ...
            <span class="btn btn-success" onclick="like()"><i class="fa fa-thumbs-up"></i> {{$link->like}}</span> -
            <span class="btn btn-danger" onclick="unLike()"><i class="fa fa-thumbs-down"></i> {{$link->dislike}}</span>
          </p>
  ...
  <form method="post" action="/link/{{$link->id}}/comment" style="display:none" id="showCmt">
    @csrf
    <input name="name" class="form-control" placeholder="Tên của bạn..."
      value="<?= (Auth::check()) ? Auth::user()->name : '' ?>" <?php if(Auth::check()) { echo 'readonly' ;} ?>>
    <br>
    ...
    <div class="panel-heading">
      <div class="row">
        <div class="col-md-4 text-left panel-title">
          <h3 class="panel-title"><i class="glyphicon glyphicon-comment"></i> Bình luận gần đây <span
              class="label label-success">{{ count($link->comments) }}</span></h3>
        </div>
